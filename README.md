...
The architecture sections describe **Claude Code v2.1.88**. The design guide and resource catalog extend that analysis with work reviewed through **September 15, 2026**. The main PDF link opens [arXiv v2, July 2, 2026](https://arxiv.org/abs/2604.14228v2); the [April 2026 repository PDF](./paper/Dive_into_Claude_Code.pdf) remains available as an archive.
...
Graphs can support review of completed runs. Agent communication needs rules for message delivery. Memory needs rules for recall and correction. Changes to models, prompts, or skills need explicit acceptance tests. The [September 15 source notes](./docs/agent-design-space-source-notes.md#refresh-2026-09-15) give the sources, dates, and versions.
...
| [Reducing cost and improving performance with Claude Platform](https://claude.com/blog/reducing-cost-and-improving-performance-with-claude-platform) | September 8: Anthropic describes prompt-audit, hillclimb, and cost-optimize workflows in the Claude API skill. Given an evaluation, hillclimb uses training cases to guide changes and a held-out test set to assess the final configuration. |
...
| [Programmatic tool calling](https://platform.claude.com/docs/en/agents-and-tools/tool-use/programmatic-tool-calling) | The first beta was released in November 2025. Current documentation describes Python code that pauses for client tool results, then resumes in the same container. The client returns the container ID with the results. The model receives the final program output, so the program can process raw tool data first. |
...
| [Cloud Agents and Cursor Harness Improvements](https://cursor.com/changelog/08-19-26), [Self-hosted machines](https://cursor.com/changelog/self-hosted-machines), [Projects](https://cursor.com/changelog/projects) | Cursor | The August 19 update adds event subscriptions, persistent goals, and subagents on separate machines. The September 2 update adds scheduling across named worker pools. Together they motivate separating the lifetime of a goal, a session, and its execution machine; the announcements do not establish exactly-once event or result delivery. Projects entered beta on September 10. A cloud coordinator plans work, delegates implementation, and returns results for user review. A project synchronizes a set of files across cloud and local agents. |
| [Introducing the Agents API](https://openai.com/index/introducing-the-agents-api/) | OpenAI | The September 10 public beta exposes the Codex harness as a managed service. Applications provide tools and choose an execution environment. OpenAI runs the agent loop and manages sessions, context, and recovery. |
| [Mastra Factory beta](https://mastra.ai/blog/announcing-mastra-factory-beta) | Mastra | Entered beta on September 8. Current documentation separates stage transitions, entry and exit actions, and transition approval. PR review and task completion are tracked separately. |
| [Rethinking skills and prompts for GPT-6 Astra](https://developers.openai.com/blog/rethinking-skills-and-prompts-for-gpt-6-astra) | OpenAI | September 11 guidance for GPT-6 Astra recommends short skill descriptions, task-specific document references, and explicit completion conditions. Codex can shorten descriptions when the skill catalog is large. Old instructions can add needless work or cause an early stop. |
| [Managed marketplace updates](https://github.blog/changelog/2026-08-26-enterprise-managed-settings-now-support-autoupdate-for-plugin-marketplaces/), [Content exclusions in app and CLI](https://github.blog/changelog/2026-09-02-content-exclusions-generally-available-in-copilot-app-and-cli/), [Managed operation permissions](https://github.blog/changelog/2026-09-09-enterprise-managed-permissions-for-github-copilot-agent-operations/) | GitHub | Adds controls over plugin-marketplace updates and, for Business/Enterprise app and CLI users, excluded context. These govern different parts of the system. Content exclusion has client and input-path limits and is not a general filesystem sandbox. Managed operation permissions became generally available on September 9 for Business/Enterprise app, CLI, and VS Code Agent Host sessions. Rules use deny > ask > allow, and declared allowlists are intersected. Each managed ask needs new approval. |
...
| [Devin Fusion](https://cognition.com/blog/devin-fusion) and [Agentic MapReduce](https://devin.ai/blog/agentic-map-reduce/), [Fusion on Desktop and CLI](https://cognition.com/blog/local-fusion) | Cognition | Fusion couples two mechanisms usually treated separately: a classifier re-scores task difficulty mid-session, and the model swap is timed to land on context compaction, to avoid an extra cache miss from the switch. MapReduce draws a clean line between agents and deterministic compute: put an agent only where reasoning is needed, make everything else deterministic. Fusion reached Devin Desktop and CLI on September 11. The design account describes model-pair choices about task brief detail, whether workers can question instructions, and exploration duties. |
...
| [MCP 2026-07-28 specification](https://modelcontextprotocol.io/specification/2026-07-28/changelog) and [Enterprise-Managed Authorization](https://blog.modelcontextprotocol.io/posts/enterprise-managed-auth/) | MCP | Removes protocol-level sessions and the initialize handshake; capability discovery uses server/discover. Multi Round Trip Requests let a logical tool operation request input and continue across separate requests. Application state and continuation state still need an owner. Roots, sampling, and logging are deprecated; Tasks move to an extension. Enterprise-managed authorization provides a separate organization-level control. |
...
| [Agent Host and the Agent Host Protocol](https://code.visualstudio.com/updates/v1_129), [VS Code 1.137](https://code.visualstudio.com/updates/v1_137) | Microsoft | VS Code 1.129 moves agent sessions out of the editor into a dedicated process addressed over an open Agent Host Protocol. Because the session lives in its own process it keeps running with no client attached and can be rendered from several windows at once, and one host runs Copilot, Claude, and Codex behind a single session model. Agents on the host can list other sessions, read their transcripts, spawn a session to hand off a subtask, and message another session, with sends requiring user confirmation and a burst cap so one request cannot fan out into unbounded sessions. Version 1.137 queues agent messages to a busy chat and starts them in send order after the active turn succeeds. Automations is in Preview and needs an awake machine and the relevant running host. Disabling a schedule leaves its active run in progress. |
| [Gemini Enterprise Agent Platform](https://docs.cloud.google.com/gemini-enterprise-agent-platform/release-notes) | Google | Computer Use and Shell sandboxes became generally available on September 9. Each Shell call starts a new shell. Separately, VPC Service Controls support for Agent Gateway requires ALL_TRAFFIC and a gateway created after September 8 with an agent connectivity template. |
...
|:--------|:----------------------|
| [APPA: Recoverable Information-Flow Control for Real-World LLM Agents](https://arxiv.org/abs/2607.24625v2) | The August 26 revision checks tool calls before execution and checks returned data before it enters context. It uses temporary branches to inspect untrusted data and restricts what those branches can return. |
| [Twin Agent: Context Residual Compression for Privilege Separated Agents](https://arxiv.org/abs/2607.19595v1) | Assigns untrusted reading and privileged execution to different agents. The reader sends compact hints to the executor. This makes observation access, action authority, and the return channel separate design choices. |
| [MemSecBench: Tracking Agent Memory Poisoning from Persistence to Consequence and Repair](https://arxiv.org/abs/2607.27080v1) | Tests whether malicious content is stored, later changes an action, and can be removed while useful memory is retained. Compares complete agent, memory-backend, and model configurations. |
...
| [AWS Security Agent bucket ownership advisories](https://aws.amazon.com/security/security-bulletins/2026-105-aws/) | AWS disclosed CVE-2026-87912 and CVE-2026-87913 on September 10, 2026. They concern missing bucket ownership checks when uploading workspace archives. Fixed plugin 1.1.0 and MCP server 0.2.0 check the expected owner and stop on a mismatch. |
...
| [LoopsBench: From Harness Engineering to Loop Engineering in Coding Agent Evaluation](https://arxiv.org/abs/2608.00267v1) | arXiv | Evaluates sustained development rather than one issue and one terminal state. Its 112 tasks are dependency DAGs over more than 5,300 separately testable development units; all unit tests are visible from the start. The evaluator activates them for scoring as prerequisites pass and keeps passed tests as regression obligations. The best tested pairing — Opus 4.7, Claude Code, and an outer continuation loop — resolves 25% of tasks, while recorded agent plans recover only part of the source-derived dependency structure. This is the first benchmark in the catalog aimed explicitly at the loop outside the agent loop. |
...
| [SWE Refactor Bench: Can Coding Agents Complete a Long-Horizon, Whole-Repository Stack Migration?](https://arxiv.org/abs/2608.23564v1) | arXiv, August 24, 2026 | Checks the requested stack migration, fixed behavioral tests, and generated behavioral counterexamples separately. A counterexample must pass on the original, fail on the submission, and reproduce repeatedly. |
...
|:------|:------|:----------|
| [Living-Harness Is an Interactive-Agent Evolver](https://arxiv.org/abs/2607.26598v2) | arXiv, August 11 revision | After an episode is scored, updates procedural memory and a state graph for later retrieval. The graph guides future decisions; it does not directly execute them. Tools and base context remain fixed. |
| [Self-Evolving Coding Agents](https://arxiv.org/abs/2608.03392v3) | Survey, August 29 revision | Organizes persistent changes by their target, timing, and feedback. Separates changes to memory, skills, tools, workflows, components, and context, and reviews how their effects are evaluated. |
| [What Does Multi-Harness RL Learn? Credit Assignment and Portability in Coding Agents](https://arxiv.org/abs/2609.04518v1) | arXiv, September 3 | Holds training records and budget fixed while changing whether reward comparisons stay within each task–harness pair or span harnesses. Tests the trained models with a harness absent from training; cross-harness grouping alone did not improve transfer in this study. |
...
| [From Review to Reuse: How Post-Task Workflow Can Support Human-AI Agent Interaction](https://arxiv.org/abs/2609.13136v1) | arXiv, September 11, 2026 | Trace2Flow turns a completed run into an editable workflow graph. Users can inspect step inputs and outputs, change dependencies, and rerun steps for a related task. The graph links its steps to the recorded execution. |
| [Decoupling Readiness from Release for Tail-Aware Scheduling of Agentic LLM Workflows](https://arxiv.org/abs/2609.10964v1) | arXiv, September 10, 2026 | Separates dependency readiness from submission to a shared inference engine. The scheduler chooses which turn to submit and limits submitted work that has not finished. Trace replay measures workflow tail latency under resource contention. |
| [TROVE: Adaptive Agent Skill Orchestration via Trace-Grounded Route Validation and Editing](https://arxiv.org/abs/2609.05019v1) | arXiv, September 4, 2026 | After each skill runs, the controller uses its result to keep the next step, insert a local response, or replace the remaining route. Completed work and its artifacts remain available. |
...
| [Defining AI Agents: A Compendium of Criteria, Metrics, and Benchmarks](https://arxiv.org/abs/2609.11018v1) | arXiv, September 10, 2026 | Maps environmental interaction, learning and adaptation, autonomy, goal-directed behavior, and temporal coherence to existing metrics and benchmarks. It distinguishes independence from human input from task performance to help builders select the capabilities to evaluate. |
...
| [SkillAdam: Stable and Efficient Skill Evolution for Agents](https://arxiv.org/abs/2609.08944v1) | arXiv, September 8, 2026 | Keeps a history of problems and attempted fixes. Variation in case results guides the next edit budget. Acceptance checks protected metrics on the same sampled cases; final tests remain separate. |
| [Skill Issue: Lessons from Optimizing Repository SKILLs for Coding Agents](https://arxiv.org/abs/2609.12742v1) | arXiv, September 11, 2026 | Builds tasks by reverting the implementation changes from merged PRs at a fixed repository commit, while keeping its tests. It compares runs with and without an optimized SKILL.md. Tests on three Kotlin repositories could not separate observed gains from run-to-run variation. |
...
| [Co-Evolving Harnesses and Models: On-Policy Correction Helps Weaker Models Catch Up Where Imitation Fails](https://arxiv.org/abs/2609.09134v1) | arXiv, September 8, 2026 | Tests fine-tuning after harness evolution on seven enterprise tasks. The authors report regressions after training on full expert trajectories. The alternative starts from the weaker model’s own runs and replaces one failing turn with an expert correction before training. |
...
| [Resume Means Resume: A Machine-Checked Conformance Contract for Checkpoint, Interrupt, and Resume Semantics in Workflow Persistence Layers](https://arxiv.org/abs/2608.03836v3) | arXiv, August 8 revision | Defines six properties for checkpoint, interrupt, and resume behavior. Tests fixed versions of five frameworks for repeated effects, conflicting resume inputs, invalid checkpoints, and concurrent recovery. Distinguishes documented replay behavior from contract violations. |
...
| [**Codex local memories**](https://learn.chatgpt.com/docs/customization/memories), [memory v2 source](https://github.com/openai/codex/commit/2cbbf0c9b542a36a1c3284b5e804917635b6f666) | Reviewed September 2026 | When enabled, eligible prior sessions feed background extraction and consolidation. A short index guides retrieval of detailed memory. This local store is separate from ChatGPT web memory and from experimental working-context notes; [summarizing compaction also remains in v0.153.4](https://github.com/openai/codex/blob/3d2ee51ca2d5db578f328aa75e20aa22c0197c9a/codex-rs/core/src/compact.rs). The September 8 mainline implementation adds a separate memory v2 store and optional writes to both versions. The selected version supplies context. V2 prioritizes user messages and answers to agent questions within the extraction input budget. Its prompts require task corrections to stay within that task’s scope. Stable 0.154.0 lacks the v2 selectors. |
| [Claude Tag memory scope](https://github.com/anthropics/claude-code/releases/tag/v2.1.268) | September 10, 2026 | Each public channel keeps its own notes. Claude no longer recalls notes from other public channels; workspace notes remain shared. This defines which notes can enter the current channel’s context. |
| [Hermes background memory review](https://github.com/NousResearch/hermes-agent/releases/tag/v2026.9.11) | v0.21.2, September 11, 2026 | In unattended background reviews, the built-in memory tool turns replace/remove operations, including a batch that contains them, into an approval proposal. It blocks that call from applying the changes. Proposal storage is best effort. |
